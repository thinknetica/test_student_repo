...
Все задачи, которые вам нужно будет сделать, оформляются как issues на гитхабе. Вы можете посмотреть их на вкладке Issues своего репозитория, но удобнее будет работать через Project Board (вкладка Projects), где задачи будут представлены в более удобном виде (об этом ниже).
